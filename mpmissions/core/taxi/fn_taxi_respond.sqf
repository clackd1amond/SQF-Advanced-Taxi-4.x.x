...
switch (_mode) do
{
	...
	case 2:
	{
		hint format ["Taxidriver %1 accepted your call, he is currently %2 meters away from your position.",name _info,floor (player distance _info)];
		...
		while {life_calledTaxi && !(player getVariable "restrained")} do {
			...
			hintSilent parseText format ["<t size='3'><t color='#FF0000'>TAXI-METER</t></t> <br/><br/><t align='left'>Distance driven</t><t align='right'>%1</t><br/><t align='left'>Price</t><t align='right'>%2</t>",floor (life_taxiTotalPrice/pricePerM),life_taxiTotalPrice];
	...
	{		
			...
			if (life_atmcash<life_taxiTotalPrice) then {life_atmcash = 0;} else { life_atmcash = life_atmcash - life_taxiTotalPrice;};
			[[life_taxiTotalPrice,pricePerM,2],"life_fnc_finishRide",life_taxiDriver,false] spawn life_fnc_mp;
	};
};